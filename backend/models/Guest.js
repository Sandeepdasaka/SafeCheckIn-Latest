...
// Get photo URL - POLICE ONLY. This must never be surfaced to a hotel/
// receptionist client: it points at routes/policeGuestPhotoRoutes.js,
// which requires a police JWT and audit-logs every view. Do not build a
// hotel-facing equivalent - see controllers/guestController.js's
// `sanitizeGuestPhotos`, which strips photo data out of every hotel
// response instead.
guestSchema.methods.getPhotoUrl = function (photoType) {
  ...
  return `/api/police/guests/${this._id}/photo/${photoType}`;
};
