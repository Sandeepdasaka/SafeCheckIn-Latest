...
const sanitizeGuestForHotel = (guestDoc) => {
  ...
  if (plain.photos) {
    ...
    for (const field of PHOTO_FIELDS) {
      const photo = plain.photos[field];
      if (photo && (photo.data || photo.path)) {
        safePhotos[field] = {
          captured: true,
          mimeType: photo.mimeType,
          uploadTime: photo.uploadTime,
          // No `data`, no `path` - hotel staff never receive the bytes.
        };
      } else {
        safePhotos[field] = { captured: false };
      }
    }
...
const getAllGuests = async (req, res) => {
  try {
    ...
    res.json({
      success: true,
      guests: sanitizeGuestListForHotel(populatedGuests),
      pagination: {
...
const getGuestByRoom = async (req, res) => {
  try {
    ...
    res.json({
      success: true,
      data: sanitizeGuestListForHotel(guests),
      count: guests.length,
    });
  } catch (error) {
